...
declare global {
    interface Window {
        goIwm: { // Implemented methods in the wasm
            midiStringifyTracks(encodedMidi: string): `${typeof TRACKS_NORMAL_PREFIX}${string}` | string;
            midiToEventedObjects(encodedMidi: string, encodedTrackConfigs: string): string;
        },
    }
}
...
type DotNestedKeys<T> = {
    ...
    ? `${K}` | `${K}.${number}` | `${K}.${number}.${DotNestedKeys<T[K][number]>}`
    : never
    : T[K] extends object
    ? K extends string | number
...
const InstructionConfigHint = (description: React.JSX.Element, literalType?: InstructionConfigHintLiteralType) =>
    ...
    >
        ...
        >
            <Icon icon="fa:info-circle" style={{ marginRight: "10px" }} />
        </span>
    </OverlayTrigger>;
...
export default function App() {
    ...
    const handleTrackConfigChange = (event: HandleableEvents, index: number) => {
        ...
        if (keys.length > 1) {
            ...
            updatedTracks[index] = {
                ...
                [key1]: {
                    ...(updatedTracks[index][key1] as Record<string, any>),
    ...
    const addDrumConfig = (index: number) => {
    ...
    const removeDrumConfig = (trackIndex: number, drumIndex: number) => {
        const newTrackConfigs = [...trackConfigs];

        newTrackConfigs[trackIndex].drumSplit.splice(drumIndex, 1);
    ...
    const handleSubmit = async (event: SyntheticEvent) => {
        ...
        switch (submitterId) {
            case "submit-config": {
                ...
                const result = window.goIwm.midiToEventedObjects(
            ...
            <div className="text-center mt-2">
                ...
                <input
                    ...
                    onInput={async function (event) {
                        ...
                        if (file) {
                            let result = window.goIwm.midiStringifyTracks(await readFileAsBase64(file));
                ...
                {trackConfigs.map((track, index) => (
                    selectedTrackConfig.id === track.id && (
                        <div key={track.id} className="track-config mb-4">
                            ...
                            {/* Loop Configuration */}
                            <h4 style={{ display: "flex", alignItems: "center" }}>
                                <Icon icon="gravity-ui:arrows-rotate-left" style={{ marginRight: "10px" }} />
                            ...
                            <div className="row mb-3">
                                ...
                                <div className="col">
                                    <div className="d-flex justify-content-between">
                                        <label htmlFor={`loop.offset-${track.id}`} className="form-label">Loop Offset</label>
                                    ...
                                    <input
                                        type="number"
                                        className="form-control configuration-string-inputter"
                                        id={`loop.offset-${track.id}`}
                                        name={"loop.offset" satisfies DotNestedTrackConfigKeys}
                                        value={track.loop.offset}
                                        onChange={(event) => handleTrackConfigChange(event, index)}
                                    />
                                </div>
                            </div>
            ...
            >
                ...
                <div id="about-more-modal-container" style={{
                    ...
                    <div>
                        ...
                        V_{t,n} = \mathrm{min}_{\mathfrak{V}} + \left\lbrace \left( \frac{v_{t}(n)}{\mathrm{max}_{v}} \right)^{L} (\mathrm{max}_{\mathfrak{V}} - \mathrm{min}_{\mathfrak{V}}) \right\rbrace, \\[0.5em]
